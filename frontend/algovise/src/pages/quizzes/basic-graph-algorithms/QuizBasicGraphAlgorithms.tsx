...
const QuizBasicGraphAlgorithms: React.FC = () => {
  ...
  useEffect(() => {
    fetch("/quizzes/basic-graph-algorithms/basic-graph-algorithms.json")
    ...
    if (allCorrect) {
      ...
      try {
        const response = await fetch(
          `http://localhost:8080/completed-quizzes?userId=${userId}&quizId=2`,
          {
            method: "POST",
            headers: {
...
export default QuizBasicGraphAlgorithms;
