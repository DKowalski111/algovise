...
const QuizFundamentalsOfGraphTheory: React.FC = () => {
  const [quizData, setQuizData] = useState<QuizData | null>(null);
  const [selectedAnswers, setSelectedAnswers] = useState<Map<number, number | null>>(new Map());
  const [submitted, setSubmitted] = useState(false);

  useEffect(() => {
    fetch("/quizzes/fundamentals-of-graph-theory/fundamentals-of-graph-theory.json")
    ...
    if (allCorrect) {
      ...
      try {
        const response = await fetch(
          `http://localhost:8080/completed-quizzes?userId=${userId}&quizId=${1}`,
          {
            method: "POST",
            headers: {
...
export default QuizFundamentalsOfGraphTheory;
